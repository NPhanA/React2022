...
export default function TeacherManager() {
    ...
    const editFormChange = (event)=> {
        ...
        const fieldAge = event.target.getAttribute("age");
        const fieldAddress = event.target.getAttribute("address");
        const fieldPhoneNumber = event.target.getAttribute("phoneNumber");
        const fieldTeachedClass = event.target.getAttribute("teachedClass");
        const fieldValue = event.target.value;
        const newFormData = {...editFormData};
        newFormData[fieldName] = fieldValue;
        newFormData[fieldAge] = fieldValue;
        newFormData[fieldAddress] = fieldValue;
        newFormData[fieldPhoneNumber] = fieldValue;
        newFormData[fieldTeachedClass] = fieldValue;
    ...
    const handleEditClick = (event, contact) => {
        ...
        const formValue  = {
            name: contact.name,
            id: contact.id, age: contact.age, address: contact.address, phoneNumber: contact.phoneNumber, teachedClass: contact.teachedClass
        }
                                    ...
                                    <tbody>
                                        {
                                            teacher.map((currentValue, index) => (
                                                <Fragment>
                                                    {editContactID === currentValue.id ? <TableRowInput onChange={editFormChange} numbers={index} Age={editFormData.age} Address={editFormData.address} name={editFormData.name} PhoneNumber={editFormData.phoneNumber} TeachedClass={editFormData.teachedClass}></TableRowInput> : <TableRow change={(event) => handleEditClick(event, currentValue)} key={index} name={currentValue.name} numbers={index} age={currentValue.age} address={currentValue.address} phoneNumber={currentValue.phoneNumber} teachedClass={currentValue.teachedClass} enter={handleDeleteClick}></TableRow>}
                                                </Fragment>
