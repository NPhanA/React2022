...
export default function TeacherManager() {
    ...
    const handleEditFormSubmit = (event,classTeached) => {
        event.preventDefault();
        const editedContact = {
            id: editFormData.id,
        ...
        const newteacher = [...teacher];
        const index = teacher.findIndex((contact) => contact.id === classTeached);
                                    ...
                                    <tbody>
                                        {
                                            teacher.map((currentValue, index) => (
                                                <Fragment >
                                                    {editContactID === currentValue.id ? <TableRowInput onChange={editFormChange} onSubmit={(event)=>handleEditFormSubmit(event,currentValue.id)}  numbers={index} age={currentValue.age} address={currentValue.address} name={currentValue.name} phoneNumber={currentValue.phoneNumber} teachedClass={currentValue.teachedClass} ></TableRowInput> : <TableRow change={(event) => handleEditClick(event, currentValue)} key={index} name={currentValue.name} numbers={index} age={currentValue.age} address={currentValue.address} phoneNumber={currentValue.phoneNumber} teachedClass={currentValue.teachedClass} enter={() => handleDeleteClick(currentValue.id)}></TableRow>}
                                                </Fragment>
