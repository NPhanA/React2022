...
export default function TeacherManager() {
    ...
    const addTeacher = () => {
        ...
        let objectLength = teacher.length+1;
        console.log(objectLength);
        if (currValue.length == 0 || currDate.length == 0 || currAddress.length == 0 || currNumbers.length == 0 || currClass.length == 0) {
            alert("Please enter all field");
        } else {
            setTeachers([...teacher, {id: 0,name: currValue, age: currDate, address: currAddress, phoneNumber: currNumbers, teachedClass: currClass }])
        }   
    }
