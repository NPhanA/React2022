...
export default function TeacherManager() {
    let [teacher, setTeachers] = useState([
        ...
        { id: 1, name: "John Smith", age: 31, address: "London", phoneNumber: "0918321", teachedClass: "English" },
        { id: 2, name: "John Smith", age: 31, address: "London", phoneNumber: "0918321", teachedClass: "English" },
        { id: 3, name: "John Smith", age: 31, address: "London", phoneNumber: "0918321", teachedClass: "English" },
        { id: 4, name: "John Smith", age: 31, address: "London", phoneNumber: "0918321", teachedClass: "English" },
        { id: 5, name: "John Smith", age: 31, address: "London", phoneNumber: "0918321", teachedClass: "English" },
    ])
    ...
    const addTeacher = () => {
        ...
        let currId = teacher.length;
        let parseID = teacher[currId].id;
        if (currValue.length == 0 || currDate.length == 0 || currAddress.length == 0 || currNumbers.length == 0 || currClass.length == 0) {
            alert("Please enter all field");
        } else {
            setTeachers([...teacher, { id:parseID,name: currValue, age: currDate, address: currAddress, phoneNumber: currNumbers, teachedClass: currClass }])
        }
    }
                                    ...
                                    <tbody>
                                        {
                                            teacher.map((currentValue, index) => (
                                                <Fragment >
                                                    {editContactID === currentValue.id ? <TableRowInput onChange={editFormChange} onSubmit={handleEditFormSubmit}  numbers={index} age={currentValue.age} address={currentValue.address} name={currentValue.name} phoneNumber={currentValue.phoneNumber} teachedClass={currentValue.teachedClass} ></TableRowInput> : <TableRow change={(event) => handleEditClick(event, currentValue)} key={index} name={currentValue.name} numbers={index} age={currentValue.age} address={currentValue.address} phoneNumber={currentValue.phoneNumber} teachedClass={currentValue.teachedClass} enter={() => handleDeleteClick(currentValue.id)}></TableRow>}
                                                </Fragment>
