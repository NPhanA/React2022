...
export default function TeacherManager() {
    ...
    const handleEditFormSubmit = (event,contactId) => {
        event.preventDefault();
        const editedContact = {
                                    ...
                                    <tbody>
                                        {
                                            teacher.map((currentValue, index) => (
                                                <Fragment>
                                                    {editContactID === currentValue.id ? <TableRowInput onChange={editFormChange} onSubmit={()=>handleEditFormSubmit(currentValue.id)} numbers={index} age={editFormData.age} address={editFormData.address} name={editFormData.name} phoneNumber={editFormData.phoneNumber} teachedClass={editFormData.teachedClass}></TableRowInput> : <TableRow change={(event) => handleEditClick(event, currentValue)} key={index} name={currentValue.name} numbers={index} age={currentValue.age} address={currentValue.address} phoneNumber={currentValue.phoneNumber} teachedClass={currentValue.teachedClass} enter={() => handleDeleteClick(currentValue.id)}></TableRow>}
                                                </Fragment>
